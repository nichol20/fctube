...
import (
	...
	"github.com/nichol20/fctube/internal/converter"
	"github.com/nichol20/fctube/pkg/rabbitmq"
...
func TestTaskProcessing(t *testing.T) {
	...
	videoConverter.HandleMessage(ctx, <-msgs, exchangeName, finishConversionKey, finishConversionQueue)
	...
	case msg := <-confirmationMsgs:
		...
		assert.Equal(t, videoTask.Path, confirmation.Path)
		msg.Ack(false)
